...
async def test_json_source_get_positions(json_source):
    ...
    nvda_put = next(p for p in positions if p.instrument.symbol == "NVDA" and p.instrument.option_details is not None)
...
async def test_json_source_negative_positions(json_source):
    ...
    amd_put = next(p for p in positions if p.instrument.symbol == "AMD" and p.instrument.option_details is not None)
